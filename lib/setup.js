...
var cloneQueue = async.queue(function (job, done) {
  ...
  shell.exec(util.format('git clone %s %s', url, dir), function (code, output) {
    if (code !== 0) {
  ...
  fs.mkdir(repo, function (err) {
    request({
      ...
      async.parallel(jobs, function (err) {
        ...
        shell.exec(util.format(cmd, repo, repo, repo, repo, repo), function (code) {
